...
clear all
Screen('CloseAll');
addpath('C:\Users\biosemieeg\Desktop\jac\Leverhulme\old_code');
...
imgpath=('C:\Users\biosemieeg\Desktop\jac\Leverhulme\EEGimages3');
...
savefile4 = [(subject) '_AB_temp_output.mat'];
fullscreen=input('Fullscreen? y/n    ','s');
% tiles=input('number of tiles? 2/4/6    ','s');
tiles='4';

load ('C:\Users\biosemieeg\Desktop\jac\Leverhulme\EEGimages3\blankforeeg.mat');
load ('C:\Users\biosemieeg\Desktop\jac\Leverhulme\EEGimages3\triforeeg.mat');
%% ------------------------------------------------------------------------
...
%   screen.scrhei=round(1023); %(900); %;
   screen.scrwid=round(1900); %;  %%% width and height of the screen in pixels (800x600 it smaller so you can crash when testing code)
  screen.scrhei=round(1000); %;
...
%% ----------------------------------------------------------------------
param.notrials=348; %%% must be devisible by 2 and 3

param.precuelen=0.6; %%% sec pre cue length
param.cuelen=0.6; %%% sec cue on before stimulus
param.cuegap=1.0;
param.targetlength=0.1; %%% sec  stimulus length
param.intertrlength=2.5;  %%% sec post stimulus length
...
%%% Stimuli info
%% ---------------------------------------------------------
%%%% select images from bins- should have 87 stim.
stimchoice=[];
blankmatch=[];
updown=[];
hm=[];
...
expoutput=zeros(param.notrials,19);
...
%%% recap instructions
%%% ----------------------------------------------------------------------
fd = fopen(['instructionsRH.txt'], 'rt');
...
% % You pressed key 27 which is ESCAPE
    ...
    KeyCodes.Escape=27;
    KeyCodes.Space=32;
    KeyCodes.x=219;    %88; index
    KeyCodes.z=221;    %90; middle finger
...
%          EEG ping
    ...
    if i==param.f_no-33 && output.response==0;
     while ~any(Key); [Keypr KeyTime Key] = KbCheck; end; %%%% wait for press
     while any(Key); [Keypr KeyTime Key]= KbCheck; end; %%% wait for release
    end
    
    %%% get rid of all extra textures.
       if i==param.f_no-35; %send if it's top or bottom
     ...
     if i==param.f_no-31; %send if it's a congruent trial or not
     ...
     if i==param.f_no-27; %send if it's a correct trial
        ...
        if i==param.f_no-24; %sind if side cue was correct 
        ...
        if i==param.f_no-21; % send cue side
        ...
        if i==param.f_no-18; %send side stim on. 
    ...
    KeyCodes.one=97;   %or 49
    KeyCodes.two=98;   %0r 50
    KeyCodes.three=99; %or 51
    Screen('TextSize',screen.w, 20);
    text= 'Guessed!  Fairly Sure    Certain\n\n  1        2          3';    
         
   [nx, ny, bbox] = DrawFormattedText(screen.w, text,  'center', ((screen.height)/2)-35, 200, 150);
   Screen('Flip',screen.w);
   [firstPress]= KbQueueDemoPart1();
   find(firstPress);
   WaitSecs(.5);
   output.certain=find(firstPress);
      ...
      expoutput(count,19)=output.certain;
...
RT.Properties.VariableNames = {'tr','count','tri_start','cue_start','stim_start','resptimeABS','responsetime','UDcuecong','response','sidecue','sidestim','cuecong','hz','canny','hmax','rot','cueUD','targUD','certain'};
